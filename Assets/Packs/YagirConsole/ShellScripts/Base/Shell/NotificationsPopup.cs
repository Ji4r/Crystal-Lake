...
namespace ConsoleShell
{
    public class NotificationsPopup : MonoBehaviour
    {
        ...
        {
            animationProgress = 0;
            var time = animationCurve.keys.Last().time;
            while (animationProgress < time)
            {
                yield return null;
                animationProgress += Time.unscaledDeltaTime * animationSpeed;
                popup.anchoredPosition = new Vector2(Mathf.Lerp(minValue, maxValue, animationCurve.Evaluate(animationProgress)), popup.anchoredPosition.y);
            }
        }
    }
}
